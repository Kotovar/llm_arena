...
export function footerText(todos) {
  return countLabel(todos.visible().length);
}
...
export function mountTodos(root, initial) {
  ...
  function render() {
    ...
    for (const button of filters) button.setAttribute("aria-pressed", String(button.dataset.filter === todos.getFilter()));
  }

  for (const button of filters) button.addEventListener("click", () => todos.setFilter(button.dataset.filter));
