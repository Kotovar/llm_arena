...
// Гонка настоящая: модель меняют ровно в тот момент, когда helper уже прочитал исходный файл и
// ждёт ответа сервера. Подмена делается из обработчика запроса — это надёжный барьер, а не таймер.
it("останавливает сеанс, если во время ожидания сервера активировали другую модель", async () => {
  const session = piSession(null, 32_000);
  ...
  try {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("активирована другая модель");
    // Окно новой модели уцелело.
