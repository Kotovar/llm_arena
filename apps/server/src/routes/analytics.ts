...
type PointMeta = {
  ...
  tag: string | null; peakVramMiB: number | null; modelParams: string | null; estimatedCostPerRun: number | null;
};
  ...
  app.get<{ Querystring: SliceQuery }>("/api/analytics/decision-points", async (request) => {
    ...
    for (const { row, metricRow } of enrichedRows(store, slice)) {
      ...
      if (!points.has(key)) {
        ...
        points.set(key, {
          ...
          tag: slice.tag ?? null,
          peakVramMiB: null,
          // Число параметров — не то же самое, что пик VRAM за прогон, и в аналитике это разные колонки.
    ...
    return aggregateModelStats(rows)
      .map((stats) => {
        ...
        const { peakVramMiB, modelParams, estimatedCostPerRun, ...meta } = points.get(stats.key)!;
        return {
          ...meta,
