...
function NewProfileForm({ modelId, source, layerCount, pending, create }: { modelId: string; source: Profile; layerCount: number | undefined; pending: boolean; create: (input: { modelId: string; name: string; parameters: LlamaParameters }) => Promise<Profile> }) {
  ...
  return <details className="manual-profile" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}><summary>Добавить профиль</summary><form className="form-grid" onSubmit={(event) => {
    ...
    <label>Минимальный контекст<NumberField name="fitContextMin" min="4096" defaultValue={source.parameters.fitContextMin ?? 100000} required /></label>
    <label>Температура<NumberField name="temperature" min="0" max="2" step={0.05} defaultValue={source.parameters.temperature ?? DEFAULT_LLAMA_TEMPERATURE} required /></label>
    <label>Seed<NumberField name="seed" defaultValue={source.parameters.seed ?? ""} placeholder="случайный" /></label>
      ...
      <Panel title={kind === "local-gguf" ? "Добавить локальную модель" : "Добавить облачную модель"}>
        {kind === "local-gguf" ? <form onSubmit={submitLocal} className="form-grid">
          ...
          <fieldset className="sampling-fields span-2"><legend>Сэмплинг</legend>
            <label>Температура<NumberField name="temperature" min="0" max="2" step={0.05} defaultValue={DEFAULT_LLAMA_TEMPERATURE} required /><small>Насколько модель отходит от самого вероятного продолжения. Ниже — стабильнее и повторяемее, выше — разнообразнее.</small></label>
            <label>Seed<NumberField name="seed" placeholder="случайный" /><small>Фиксирует случайность генерации: с одинаковым seed и температурой прогон повторяем. Пусто — llama.cpp выбирает сам.</small></label>
          </fieldset>
