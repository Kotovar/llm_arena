...
export function Launcher() {
  ...
  const usingOmpAgent = isLocalModel && useOmpAgent && Boolean(ompRunner);
  ...
  return <Page title="Запустить проверку модели" eyebrow="Новый запуск" intro="Выберите модель, один или несколько промптов. Остальные параметры приложение подберёт автоматически.">
    <section className="launch-card">
      ...
      <div className="launch-step" data-ready={Boolean(selectedRunner)}>
        ...
        <div className="launch-fields">
          ...
          {isLocalModel ? <fieldset className="result-mode">
            ...
            <label><input type="radio" name="localPromptMode" checked={usingOmpAgent} onChange={() => { setUseOmpAgent(true); setRunnerOverride(""); }} disabled={!ompRunner} />OMP-среда</label>
          </fieldset> : null}
        </div>
      ...
      <details className="advanced"><summary>Дополнительные настройки</summary><label>Способ запуска<select value={runnerOverride} onChange={(event) => { const runner = runners.data?.find((item) => item.id === event.currentTarget.value); setUseOmpAgent(isLocalModel && runner?.kind === "omp"); setRunnerOverride(event.currentTarget.value); }}><option value="">Автоматически: {automaticRunner?.name ?? "не определён"}</option>{runners.data?.map((runner) => <option key={runner.id} value={runner.id}>{runner.name}</option>)}</select></label></details>
