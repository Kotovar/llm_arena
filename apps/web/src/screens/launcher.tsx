...
import { chooseRunner, initializeTaskSelection, latestProfiles, modelOptionLabel, reasoningEffortsForModel } from "../ui.js";
...
export function Launcher() {
  ...
  return <Page title="Запустить проверку модели" eyebrow="Новый запуск" intro="Выберите модель, один или несколько промптов. Остальные параметры приложение подберёт автоматически.">
    <section className="launch-card">
      ...
      <div className="launch-step prompt-step"><span>2</span><fieldset className="prompt-picker"><legend><strong>Какие промпты запустить</strong><small>{selectedTasks.length} из {tasks.data?.length ?? 0}</small></legend><div className="picker-actions"><button type="button" onClick={() => setSelectedTaskIds(allTaskIds)}>Выбрать все</button><button type="button" onClick={() => setSelectedTaskIds([])}>Снять выбор</button><Link to="/tasks">Добавить промпт</Link></div><div className="prompt-options">{tasks.data?.map((task) => <label key={task.id} className={selected.has(task.currentRevision.id) ? "selected" : ""}><input type="checkbox" checked={selected.has(task.currentRevision.id)} onChange={(event) => setSelectedTaskIds((current) => event.currentTarget.checked ? [...(current ?? []), task.currentRevision.id] : (current ?? []).filter((id) => id !== task.currentRevision.id))} /><span><strong>{task.currentRevision.name}</strong><small>{task.currentRevision.prompt}</small></span></label>)}</div>{!tasks.data?.length ? <p className="empty">Сначала добавьте промпт.</p> : null}</fieldset></div>
