...
export function Launcher() {
  ...
  const visibleTasks = (tasks.data ?? []).filter((task) => matchesPromptQuery(task, promptQuery) && (!promptTag || task.currentRevision.tags.includes(promptTag)));
  const promptTags = [...new Set((tasks.data ?? []).flatMap((task) => task.currentRevision.tags))].sort((left, right) => left.localeCompare(right, "ru"));
  ...
  return <Page title="Запустить проверку модели" eyebrow="Новый запуск" intro="Выберите модель, один или несколько промптов. Остальные параметры приложение подберёт автоматически.">
    <section className="launch-card" data-empty-models={models.data?.length === 0}>
      ...
      <div className="launch-step prompt-step" data-ready={selectedTasks.length > 0}><span>2</span><fieldset className="prompt-picker"><legend><strong>Какие промпты запустить</strong><small>{selectedTasks.length} из {tasks.data?.length ?? 0}</small></legend><div className="picker-actions"><input type="search" value={promptQuery} onChange={(event) => setPromptQuery(event.currentTarget.value)} placeholder="Поиск" aria-label="Поиск промптов" /><button type="button" onClick={() => setSelectedTaskIds(visibleTasks.map((task) => task.currentRevision.id))}>Выбрать все</button><button type="button" onClick={() => setSelectedTaskIds([])}>Снять все</button><Link to="/tasks">Добавить промпт</Link></div>{promptTags.length ? <div className="prompt-tags" role="group" aria-label="Теги промптов"><button type="button" className={promptTag ? "" : "active"} aria-pressed={!promptTag} onClick={() => setPromptTag("")}>Все</button>{promptTags.map((tag) => <button type="button" key={tag} className={promptTag === tag ? "active" : ""} aria-pressed={promptTag === tag} onClick={() => setPromptTag(promptTag === tag ? "" : tag)}>{tag}</button>)}</div> : null}<div className="prompt-options">{visibleTasks.map((task) => <label key={task.id} className={selected.has(task.currentRevision.id) ? "selected" : ""}><input type="checkbox" checked={selected.has(task.currentRevision.id)} onChange={(event) => { const checked = event.currentTarget.checked; setSelectedTaskIds((current) => updateTaskSelection(current, task.currentRevision.id, checked)); }} /><span><strong>{task.currentRevision.name}</strong><small title={task.description || task.currentRevision.prompt}>{task.description || task.currentRevision.prompt}</small></span>{(() => { const note = promptCoverageNote(coverage, task, selectedModelId); return note ? <em className={`prompt-covered ${note.state}`}>{note.text}</em> : null; })()}</label>)}</div>{!tasks.data?.length ? <p className="empty">Сначала добавьте промпт.</p> : null}{tasks.data?.length && !visibleTasks.length ? <p className="empty">Ничего не нашлось по запросу.</p> : null}</fieldset></div>
