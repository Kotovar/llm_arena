...
export function Launcher() {
  ...
  const isLocalPrompt = selectedModel?.kind === "local-gguf" && resultMode === "text";
  const ompRunner = runners.data?.find((runner) => runner.kind === "omp");
  const usingOmpAgent = isLocalPrompt && useOmpAgent && Boolean(ompRunner);
  ...
  return <Page title="Запустить проверку модели" eyebrow="Новый запуск" intro="Выберите модель, один или несколько промптов. Остальные параметры приложение подберёт автоматически.">
    <section className="launch-card">
      ...
      <div className="launch-step" data-ready={Boolean(selectedRunner)}><span>3</span><div className="launch-fields"><fieldset className="result-mode"><legend>Что должна вернуть модель</legend><label><input type="radio" name="resultMode" checked={resultMode === "text"} onChange={() => { setResultMode("text"); setRunnerOverride(""); }} />Текстовый ответ</label><label><input type="radio" name="resultMode" checked={resultMode === "web"} onChange={() => { setResultMode("web"); setRunnerOverride(""); setUseOmpAgent(false); }} />Готовое web-приложение</label></fieldset>{isLocalPrompt ? <fieldset className="result-mode"><legend>Среда локальной модели</legend><label><input type="radio" name="localPromptMode" checked={!usingOmpAgent} onChange={() => { setUseOmpAgent(false); setRunnerOverride(""); }} />Голая модель</label><label><input type="radio" name="localPromptMode" checked={usingOmpAgent} onChange={() => { setUseOmpAgent(true); setRunnerOverride(""); }} disabled={!ompRunner} />OMP-среда</label></fieldset> : null}</div><span className="launch-mode-note">{resultMode === "web" ? "Будут созданы файлы и Preview" : usingOmpAgent ? "OMP: skills, расширения и настроенные MCP." : "Ответ модели без рабочей директории"}</span></div>
      <details className="advanced"><summary>Дополнительные настройки</summary><label>Способ запуска<select value={runnerOverride} onChange={(event) => { const runner = runners.data?.find((item) => item.id === event.currentTarget.value); setUseOmpAgent(isLocalPrompt && runner?.kind === "omp"); setRunnerOverride(event.currentTarget.value); }}><option value="">Автоматически: {automaticRunner?.name ?? "не определён"}</option>{runners.data?.map((runner) => <option key={runner.id} value={runner.id}>{runner.name}</option>)}</select></label></details>
