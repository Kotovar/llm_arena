...
import { chooseRunner, cloudProviderCatalogKind, galleryCoverage, type Harness, harnessLabel, initializeTaskSelection, latestProfiles, launchModeNote, launchSummary, modelOptionLabel, ompUnavailableReason, plural, promptCountLabel, reasoningEffortsForModel, effortLabel, usableHarnesses } from "../ui.js";
...
export function Launcher() {
  ...
  const blockedReason = !selectedModel ? "Подключите модель, чтобы начать."
    : !selectedTasks.length ? "Выберите хотя бы один промпт."
    : imageError ?? (isLocalModel && !activeHarnesses.length ? "Выберите доступную среду локальной модели."
      : !selectedRunner || (batchLaunch && harnessRunners.length !== harnessRuns.length) ? "Для этих параметров нет способа запуска. Проверьте модель и среду."
      : undefined);
  const dataError = models.error ?? tasks.error ?? runners.error ?? profiles.error;

  return <div className="launcher-page"><Page title="Запустить проверку модели" eyebrow="Новый запуск" intro="Выберите модель, один или несколько промптов. Остальные параметры приложение подберёт автоматически.">
    <nav className="launcher-shortcuts" aria-label="Действия с моделями"><Link to="/batch">Сравнить несколько моделей <ArrowRightIcon /></Link><Link to="/gallery">Открыть галерею результатов <ArrowRightIcon /></Link></nav>
    {dataError ? <p className="error" role="alert">Не удалось загрузить параметры запуска: {dataError.message}</p> : null}
    <section className="launch-card" data-empty-models={models.data?.length === 0}>
      <div className="launch-step" data-ready={Boolean(selectedModel)}><span aria-label="Шаг 1">1</span><div className="launch-fields"><label>Подключение<SelectMenu label="Подключение" value={selectedModelId} disabled={!models.data?.length} placeholder="Выберите модель" onSelect={(value) => { setModelId(value); setCloudModelRef(""); setProfileId(""); setHarnesses(["omp"]); setReasoningEffort(""); }} options={[{ value: "", label: "Выберите модель" }, ...(models.data ?? []).map((model) => ({ value: model.id, label: `${model.name} · ${model.provider}` }))]} /></label>{selectedModel?.kind === "local-gguf" ? <label>Профиль запуска<SelectMenu label="Профиль запуска" value={selectedProfile?.id ?? ""} onSelect={setProfileId} options={modelProfiles.map((profile) => ({ value: profile.id, label: `${profile.name} · версия ${profile.revision}` }))} /></label> : null}{selectedModel?.kind === "cloud" ? <label>Конкретная модель<SelectMenu label="Конкретная модель" value={effectiveModelRef} onSelect={(value) => { setCloudModelRef(value); setReasoningEffort(""); }} options={[...(providerCatalog?.models.some((option) => option.id === selectedModel.modelRef) ? [] : [{ value: selectedModel.modelRef ?? "", label: selectedModel.modelRef ?? "" }]), ...(providerCatalog?.models ?? []).map((option) => ({ value: option.id, label: modelOptionLabel(option) }))]} /></label> : null}{reasoningOptions.length ? <label>Уровень обдумывания<SelectMenu label="Уровень обдумывания" value={effectiveEffort} onSelect={setReasoningEffort} options={[{ value: "", label: "По умолчанию" }, ...reasoningOptions.map((effort) => ({ value: effort, label: effortLabel(effort) }))]} /><small>{selectedModel?.kind === "local-gguf" ? "Только то, что понимает шаблон чата модели." : "Показывается только для моделей с отмеченной поддержкой reasoning."}</small></label> : null}</div><Link to="/models">Подключить модель</Link></div>
      <div className="launch-step prompt-step" data-ready={selectedTasks.length > 0}><span aria-label="Шаг 2">2</span><PromptPicker tasks={tasks.data} selectedIds={selectedTaskIds} setSelectedIds={setSelectedTaskIds} coverage={coverage} modelId={selectedModelId} /></div>
      <div className="launch-step" data-ready={Boolean(selectedRunner)}>
        <span aria-label="Шаг 3">3</span>
        ...
        <p className="launch-mode-note">{modeNote}</p>
      </div>
      ...
      <div className="launch-footer"><header className="launch-review"><span className="mono">Перед запуском</span><h2>Проверка модели</h2><p>Проверьте выбранные параметры. Результат появится в истории запусков.</p></header><dl className="launch-summary" aria-label="Параметры запуска">{summary.map((item) => <div key={item.label}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl><p className="launch-readiness" data-ready={canLaunch} role="status" aria-label="Готовность запуска">{launch.isPending ? "Подготавливаем запуск…" : blockedReason ?? "Всё готово к запуску"}</p><div className="launch-action"><div><strong>{selectedTasks.length ? promptCountLabel(selectedTasks.length) : "Выберите хотя бы один промпт"}</strong><small>{imageError ?? (batchLaunch ? (harnessRunners.length === harnessRuns.length ? harnessOrder : "Для выбранных обвязок нет способа запуска") : selectedRunner ? `через ${selectedRunner.name}` : "Добавьте модель и промпт")}</small></div><button className="primary launch-button" title="Ctrl+Enter" aria-keyshortcuts="Control+Enter" onClick={startRun} disabled={!canLaunch}>{launch.isPending ? (batchLaunch ? "Создаём батч…" : "Создаём запуск…") : batchLaunch ? `Запустить ${harnessRuns.length} ${plural(harnessRuns.length, "прогон", "прогона", "прогонов")} подряд` : "Запустить"}<ArrowRightIcon /></button><small className="launch-hotkey"><kbd>Ctrl</kbd> + <kbd>Enter</kbd> — запустить</small></div></div>
