...
export function GalleryPage() {
  ...
  return <div className="gallery-page"><Page title="Галерея" eyebrow="Галерея" intro="Итоговые web-результаты.">
    ...
    {matrix.rows.length ? <Panel title="Матрица результатов" action={<span className="mono">{matrix.rows.length} × {matrix.models.length}</span>}><div className="gallery-scroll"><table className="gallery-table"><thead><tr><th scope="col" className="gallery-prompt">Промпт</th>{matrix.models.map((model) => <th scope="col" key={model.id}>{model.name}</th>)}</tr></thead><tbody>{matrix.rows.map((row) => <tr key={row.prompt.id}><th scope="row" className="gallery-prompt" title={row.prompt.description || row.prompt.prompt}><strong>{row.prompt.name}</strong>{row.prompt.description ? <small className="task-description">{row.prompt.description}</small> : <small>{row.prompt.prompt}</small>}</th>{row.cells.map((cell) => <td key={cell.model.id}><GalleryCell results={cell.results} onOpen={setOpened} /></td>)}</tr>)}</tbody></table></div></Panel> : null}
