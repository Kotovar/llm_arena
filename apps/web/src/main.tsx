...
import { chooseRunner, defaultLocalProfile, followupCountLabel, formatDuration, formatMeasuredMetric, reviewSaveLabel, runProgress, shouldFollowOutput, statusLabel } from "./ui.js";
import "./styles.css";
...
function Launcher() {
  ...
  const modelOption = providerCatalog?.models.find((option) => option.id === selectedModel?.modelRef);
  ...
  const launch = useMutation({
    mutationFn: async () => {
      ...
      const profile = profiles.data?.find((item) => item.modelId === selectedModel.id);
      return api<Run>("/runs", { method: "POST", body: JSON.stringify({ benchmarkRevisionId: benchmark.currentRevision.id, modelId: selectedModel.id, executionProfileId: profile?.id ?? null, runnerId: selectedRunner.id, resultMode, reasoningEffort: effectiveEffort || null }) });
    },
  ...
  return <Page title="Запустить проверку модели" eyebrow="Новый запуск" intro="Выберите модель и промпты. Остальные параметры приложение подберёт автоматически.">
    <section className="launch-card">
      <div className="launch-step"><span>1</span><div className="launch-fields"><label>Модель<select value={selectedModelId} onChange={(event) => { setModelId(event.target.value); setRunnerOverride(""); setReasoningEffort(""); }} disabled={!models.data?.length}><option value="">Выберите модель</option>{models.data?.map((model) => <option key={model.id} value={model.id}>{model.name} · {model.modelRef}</option>)}</select></label>{selectedModel?.kind === "cloud" ? <label>Уровень мышления<select value={effectiveEffort} onChange={(event) => setReasoningEffort(event.target.value)}><option value="">По умолчанию</option>{modelOption?.efforts.map((effort) => <option key={effort} value={effort}>{effort}</option>)}</select></label> : null}</div><Link to="/models">Подключить модель</Link></div>
